...
In the DAQ3 implementation, the RUs and the BUs are logically separate, but physically realised in the same computer node (RU/BU).
\begin{figure}[htbp]
   \centering
...
The STS  transfers the data to a cluster file system (Lustre-based), whence they are finally sent to their final destination:Tier-0, DQM or the calibration cluster.

...
The effects of radiation damage are dependent on the delivered luminosity. The dependence is modelled and parametrised exponential decay \cite{CMS-PRF-18-003}. An example of the relative laser signal in a single $i\eta$ region and layer is shown in Figure \ref{fig:ECAL-Laser_data}. 

\begin{figure}[h!]	
\centering
\includegraphics[width=\textwidth]{figures/HCAL-Laser_data2024.png} %\hfill
...
In the case of downtime of the laser system (also seen in Figure \ref{fig:ECAL-Laser_data} as for 2024), the backup approach are dedicated energy flow extrapolations, which are non-trivial. Therefore, the preference is to use laser data.

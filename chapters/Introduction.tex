...
\begin{itemize}
\item The "atom” of conditions data is the \emph{Payload}, representing the set of parameters consumed in the workflows of the physics data processing. It Associated to a user-defined C++ class in CMSSW (\texttt{CondFormats} objects)
...
Payloads and their associate IOVs. They should cover the time span required by the workload. 
\item In \texttt{cmssw}, a special C++ class (the \emph{Record}) acts as entry point for a Payload by importing database content on the \texttt{cmsRun} executable. Every processing workflow consumes a specific set of Records
